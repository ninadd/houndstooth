...
- **Free / low-cost.** Runs on free tiers end to end — Vercel Hobby, Supabase free,
  SnapTrade's free tier, and OpenRouter.
...
**Stack:** Next.js (App Router) · Supabase (Postgres + Auth + RLS) · SnapTrade · OpenRouter
(uses `~deepseek/deepseek-v4-flash-latest` by default, but you can swap in any model)
...
> with your **own** Supabase project and API keys (OpenRouter, SnapTrade). Nothing is shared
...
| **OpenRouter** | Unified API for 200+ LLMs — provides the API key for the daily AI summary | ✅ Free tier (pay-per-use) |
...
- A Supabase project (free tier is fine). SnapTrade + OpenRouter keys are optional — the app
...
In `.env.local`, set `DATA_PROVIDER=mock`. You can leave the SnapTrade/OpenRouter values
...
| `OPENROUTER_API_KEY` | server-only | AI summary | Daily AI summary won't generate (app still works fine) | OpenRouter → [api-keys page](https://openrouter.ai/keys) |
...
### 6. Set OpenRouter API key (optional)

For the daily AI summary:

1. Go to [openrouter.ai](https://openrouter.ai) and sign up.
2. Go to **Keys** → create a new API key.
3. Paste it as `OPENROUTER_API_KEY` in `.env.local`.
4. The app uses `~deepseek/deepseek-v4-flash-latest` by default (a fast, cheap model that works well for this task). You can change the model in [src/lib/openrouter.ts](src/lib/openrouter.ts) — just replace the `MODEL` constant with any OpenRouter model slug (e.g. `anthropic/claude-sonnet-5`, `google/gemini-2.5-flash`). Browse available models at [openrouter.ai/models](https://openrouter.ai/models).
...
4. Generates an AI market briefing via OpenRouter (default: DeepSeek V4 Flash).
...
SnapTrade ──webhook──→ /api/snaptrade/webhook ──→ syncUser() ──→ upsert accounts/holdings/prices
                                                                    ...
                                                                    computeAndStoreSnapshot()
                                                                        │
                                                              generateDailySummary() (if OpenRouter key set)
```
...
A: Make sure `OPENROUTER_API_KEY` is set correctly.
